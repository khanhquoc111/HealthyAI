    ...
    <div className="assessment-page hypertension-assessment">
      <div className="assessment-container">
        ...
        {showAnalysis && analysis && (
          <div className="analysis-results cardiovascular-results">
            <h3>Kết Quả Đánh Giá Nguy Cơ Bệnh Tim Mạch</h3>
            <CardiovascularAnalysisPanel analysis={analysis} />
          </div>
        )}
      </div>
    </div>
...
function CardiovascularAnalysisPanel({ analysis }) {
  if (!analysis?.risks?.items) return <p className="muted">Chưa có kết quả</p>;

  const cvdRisk = analysis.risks.items.find(r => r.key === "cardiovascular");
  if (!cvdRisk) return <p className="muted">Không tìm thấy dữ liệu bệnh tim mạch</p>;

  const riskLevel = cvdRisk.level_key;
  const riskScore = cvdRisk.score || 0;
  const maxScore = cvdRisk.max_score || 16;
  
  // ĐÃ THÊM BIẾN PERCENT Ở ĐÂY ĐỂ TRÁNH LỖI TRANG TRẮNG
    ...
    <div className={`risk-panel ${riskLevel}`}>
      ...
      <div className="risk-header">
        <div className="risk-badge">
          <span className="risk-level">{cvdRisk.level}</span>
      ...
      {/* Risk Reasons */}
      {cvdRisk.reasons?.length > 0 && (
        <div className="risk-details">
          <h4>Các Yếu Tố Nguy Cơ Phát Hiện:</h4>
          <ul className="reason-list">
            {cvdRisk.reasons.map((reason, idx) => (
              <li key={idx}>
      ...
      <div className="recommendations">
        <h4>Khuyến Cáo</h4>
        <ul>
          <li>Kiểm tra mỡ máu và huyết áp định kỳ.</li>
          <li>Tăng cường hoạt động thể chất 150 phút/tuần.</li>
          <li>Duy trì cân nặng lành mạnh (BMI 18.5-24.9).</li>
          <li>Tuyệt đối không hút thuốc lá, hạn chế rượu bia.</li>
          <li>Tăng cường rau xanh, trái cây, các loại hạt và cá chứa omega-3.</li>
          
          {/* ĐÃ ĐỔI TÊN BIẾN THÀNH cvdRisk VÀ KEY THÀNH "cao" */}
          {cvdRisk.level_key === "cao" && (
            <li className="urgent">Cần khám bác sĩ chuyên khoa tim mạch ngay để kiểm tra chuyên sâu và có hướng điều trị.</li>
          )}
        </ul>
      </div>
      ...
      {/* Diet Advice */}
      {analysis.diet && (
        <CardiovascularDietAdvice diet={analysis.diet} />
      )}
    </div>
  );
}

function CardiovascularDietAdvice({ diet }) {
  if (!diet) return null;

  return (
    <div className="diet-advice">
      <h4>🍎 Gợi Ý Dinh Dưỡng Cho Tim Mạch</h4>
