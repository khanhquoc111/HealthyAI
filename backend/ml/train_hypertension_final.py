"""
train_hypertension_final.py
---------------------------
Train model Tăng Huyết Áp từ hypertension_final.csv
Cách dùng: python backend/ml/train_hypertension_final.py
"""
import os, warnings
...
THRESH_FILENAME = "hypertension_threshold.pkl"
COLS_FILENAME   = "hypertension_feature_cols.pkl"
...
# positive rate ~35% → scale_pos_weight ~2
EXPERIMENTS = [
    {"name": "RF | Balanced", "model": RandomForestClassifier(
        n_estimators=800, max_depth=None, class_weight="balanced",
        random_state=42, n_jobs=-1)},
    *([ 
        {"name": "XGB | scale=2 depth6", "model": XGBClassifier(
            n_estimators=800, max_depth=6, learning_rate=0.05,
            subsample=0.85, colsample_bytree=0.8, scale_pos_weight=2,
            eval_metric="aucpr", random_state=42, n_jobs=-1)},
        {"name": "XGB | scale=2 depth8", "model": XGBClassifier(
            n_estimators=600, max_depth=8, learning_rate=0.04,
            subsample=0.82, colsample_bytree=0.78, gamma=0.3, scale_pos_weight=2,
            eval_metric="aucpr", random_state=42, n_jobs=-1)},
        {"name": "XGB | scale=3 depth5", "model": XGBClassifier(
            n_estimators=500, max_depth=5, learning_rate=0.06, scale_pos_weight=3,
            eval_metric="aucpr", random_state=42, n_jobs=-1)},
    ] if HAS_XGB else []),
]
...
def find_csv():
    ...
    for p in [os.path.join(root,"data","hypertension_final.csv"),
              os.path.join(script_dir,"data","hypertension_final.csv"),
              os.path.join(root,"hypertension_final.csv"),
              os.path.join(script_dir,"hypertension_final.csv")]:
        if os.path.exists(p): return p
    raise FileNotFoundError("❌ Không tìm thấy hypertension_final.csv")
...
def load_and_build():
    ...
    # Salt_Intake là binary (0/1) → proxy sodium_intake: 0=2000mg, 1=3500mg
    salt = pd.to_numeric(df["Salt_Intake"], errors="coerce")
    df["sodium_intake"] = salt.map({0: 2000.0, 1: 3500.0})

    # Không có trong dataset → NaN
    for col in ["waist","fasting_glucose","hba1c","total_cholesterol","ldl",
                "creatinine","family_hypertension","family_cardiovascular"]:
        df[col] = np.nan

    X = df[FEATURE_COLS].copy()

    if "systolic" in X.columns:
        X["systolic"] = np.nan
    if "diastolic" in X.columns:
        X["diastolic"] = np.nan
    if "hypertension" in X.columns:
        X["hypertension"] = np.nan
...
def main():
    print("="*70)
    print(f"  TRAIN MODEL — Tăng Huyết Áp ({DISEASE_NAME})")
    print(f"  Dataset: hypertension_final.csv | Features: {len(FEATURE_COLS)}")
    print("="*70+"\n")
    X,y = load_and_build()
    run_experiments(X,y)
