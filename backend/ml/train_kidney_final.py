"""
train_kidney_final.py
---------------------
Train model Bệnh Thận Mạn Tính từ ckd_final.csv
Cách dùng: python backend/ml/train_kidney_final.py
"""
import os, warnings
...
THRESH_FILENAME = "kidney_threshold.pkl"
COLS_FILENAME   = "kidney_feature_cols.pkl"
...
def find_csv():
    ...
    for p in [os.path.join(root,"data","ckd_final.csv"),
              os.path.join(script_dir,"data","ckd_final.csv"),
              os.path.join(root,"ckd_final.csv"),
              os.path.join(script_dir,"ckd_final.csv")]:
        if os.path.exists(p): return p
    raise FileNotFoundError("❌ Không tìm thấy ckd_final.csv")
...
def load_and_build():
    ...
    # Gender: 0=Nam,1=Nữ → đổi thành 1=Nam,2=Nữ (NHANES convention)
    g = pd.to_numeric(df["Gender"], errors="coerce")
    df["gender_code"]  = g.map({0: 1, 1: 2})
    df["bmi"]          = pd.to_numeric(df["BMI"], errors="coerce")

    sys_raw = pd.to_numeric(df["BloodPressure_Systolic"], errors="coerce")
    dia_raw = pd.to_numeric(df["BloodPressure_Diastolic"], errors="coerce")
    np.random.seed(42)
    df["systolic"]  = sys_raw + np.random.normal(0, 15, size=len(df))
    df["diastolic"] = dia_raw + np.random.normal(0, 10, size=len(df))
    df["hypertension"] = np.where(np.random.rand(len(df)) < 0.15, 
                                  np.random.randint(0, 2, size=len(df)), # 15% bị nhiễu
                                  (df["systolic"] >= 130) | (df["diastolic"] >= 80)).astype(float)
    df["fasting_glucose"] = pd.to_numeric(df.get("Fasting_Glucose"), errors="coerce")
    df["hba1c"]           = pd.to_numeric(df.get("HbA1c"), errors="coerce")
    df["total_cholesterol"]= pd.to_numeric(df.get("Cholesterol"), errors="coerce")
    df["creatinine"]      = pd.to_numeric(df.get("Serum_Creatinine"), errors="coerce")
    df["sodium_intake"]   = pd.to_numeric(df.get("Sodium"), errors="coerce")

    # Smoking_Status: Yes/No → 1/0
    df["smoke"]    = df["Smoking_Status"].map({"Yes": 1, "No": 0}).astype(float)
    df["exercise"] = np.nan
    df["alcohol"]  = np.nan
    df["ldl"]      = np.nan
    df["waist"]    = np.nan
    df["family_hypertension"]   = np.nan
    df["family_cardiovascular"] = np.nan

    X = df[FEATURE_COLS].copy()
    if "eGFR" in X.columns:
        X["eGFR"] = np.nan
...
def main():
    print("="*70)
    print(f"  TRAIN MODEL — Bệnh Thận Mạn Tính ({DISEASE_NAME})")
    print(f"  Dataset: ckd_final.csv | Features: {len(FEATURE_COLS)}")
    print("="*70+"\n")
    X,y = load_and_build()
    run_experiments(X,y)
