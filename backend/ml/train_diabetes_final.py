...
Cách dùng: python backend/ml/train_diabetes_final.py
"""
import os, warnings
...
EXPERIMENTS = [
    ...
    *([ 
        {"name": "XGB | scale=11", "model": XGBClassifier(
            n_estimators=800, max_depth=6, learning_rate=0.05,
            subsample=0.85, colsample_bytree=0.8, scale_pos_weight=11,
            eval_metric="aucpr", random_state=42, n_jobs=-1)},
        {"name": "XGB | scale=15 depth7", "model": XGBClassifier(
            n_estimators=600, max_depth=7, learning_rate=0.04,
            subsample=0.82, colsample_bytree=0.78, scale_pos_weight=15,
            gamma=0.3, eval_metric="aucpr", random_state=42, n_jobs=-1)},
        {"name": "XGB | scale=8 depth5", "model": XGBClassifier(
            n_estimators=500, max_depth=5, learning_rate=0.06,
            scale_pos_weight=8, eval_metric="aucpr", random_state=42, n_jobs=-1)},
    ] if HAS_XGB else []),
]
...
def find_csv():
    ...
    for p in [os.path.join(root,"data","diabetes_final.csv"),
    ...
    raise FileNotFoundError("❌ Không tìm thấy diabetes_final.csv")
...
def load_and_build():
    ...
    df["exercise"]         = np.nan
    df["alcohol"]          = np.nan
    df["sodium_intake"]    = np.nan
    df["fasting_glucose"]  = np.nan
    df["ldl"]              = np.nan
    df["creatinine"]       = np.nan
...
def main():
    print("="*70)
    print(f"  TRAIN MODEL — Đái Tháo Đường ({DISEASE_NAME})")
    print(f"  Dataset: diabetes_final.csv | Features: {len(FEATURE_COLS)}")
    print("="*70+"\n")
    X,y = load_and_build()
    run_experiments(X,y)
